...
![The device on iCUE's home screen](screenshots/icue-home.png)

On the left is what iCUE shows, on the right the same readings in Afterburner's
own overlay:

![Sensors in iCUE next to the RivaTuner overlay](screenshots/sensors.png)

The units on some of them are wrong, and that is not a bug. It is the one real
limitation of the plugin interface, explained further down. That screenshot has
every sensor switched on; the shipped settings publish only the ones iCUE can
label correctly.
...
| `[sensors] show_not_compatible` | `true` also publishes entries whose unit iCUE cannot label. Off by default. |
...
| `[log] enabled` | `1` writes a plain text log of what was published and why, to `%TEMP%\AfterburnerBridge.log`. Off by default. |
...
Percentages, MHz, MB and FPS have no label available. **Out of the box those
entries are not published at all**, so what you get is the set iCUE labels
correctly, which is no loss for load percentages since iCUE already shows CPU
and GPU load natively.

Set `show_not_compatible = true` to have them anyway. They then borrow a type
and iCUE prints that type's label next to the number: the value, the graph and
the thresholds stay correct, only the two characters after the number are
wrong, and the plugin writes the real unit into the sensor name so it reads
`Framerate (FPS)`. `[units]` decides which wrong label each one borrows.

That is what the second screenshot above shows, with everything switched on.
...
Start with the log. Set `[log] enabled = 1`, which is off by default, and the
plugin writes one line per sensor with its
